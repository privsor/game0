      ...
      <div className="mx-auto w-full max-w-md flex-1 overflow-y-auto p-4">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {data?.map((w) => (
              <div key={(w as any).userId + String((w as any).createdAt)} className="flex items-center gap-3 rounded border border-white/10 bg-white/5 p-2">
                <img src={(w as any).userImage ?? "/default-avatar.png"} alt={(w as any).userName ?? ""} className="h-8 w-8 rounded-full object-cover" />
                <div className="flex-1">
                  <div className="text-sm font-semibold truncate">{(w as any).userName ?? "User"}</div>
                  <div className="text-[11px] text-white/60">{new Date((w as any).createdAt as unknown as string).toLocaleString()}</div>
                </div>
              </div>
            ))}
          </div>
