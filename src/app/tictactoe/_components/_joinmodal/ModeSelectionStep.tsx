...
export type ModeSelectionStepProps = {
...
export function ModeSelectionStep({ 
  ...
  const DaddyButton = (
    <DaddyModeCard
      ...
      onJoinWithSocial={onJoinWithSocial}
    />
  );
  ...
  const FreeButton = (
    <button
      ...
      className={`rounded-md border px-4 py-2 text-left transition-colors ${
        joinMode === "free" 
          ? "border-slate-200 bg-slate-50/10 text-slate-100" 
          : "border-slate-500/30 bg-slate-50/5 text-slate-200 hover:border-slate-300/50"
      }`}
    >
